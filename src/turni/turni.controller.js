...
const Plessi = require("./turni.model.js");

exports.findAll = function (req, res) {
  if(req.query.hasOwnProperty("plesso")){
    Plessi.findByPlessoId(req.query.plesso,function (err, plessi) {
      if (err) res.send(err);
      res.send(plessi);
    });
  } else {
  ...
  if (req.body.constructor === Object && Object.keys(req.body).length === 0) {
    ...
    Plessi.create(new_item, function (err, valueData) {
      if (err) {
        ...
        res.json({
          error: false,
          message: "School Shift added successfully!",
          data: valueData
        });
      }
    });
    ...
    Plessi.update(
      ...
      function (err, result) {
        if (err) res.send(err);
          ...
          res.json({ error: false, message: "School Shift successfully updated", data: result });
      }
    );
  }
};
